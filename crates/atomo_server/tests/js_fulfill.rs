...
async fn js_dbquery_effect_is_fulfilled() {
    ...
    let ctx = HookContext { model_name: "Contact".into(), operation: "create".into(), data, user_id: None };
    ...
    assert_eq!(results.len(), 1, "expected one fulfilled effect, got {:?}", results);
    assert!(results[0].contains("\"ok\":true"), "dbQuery should succeed: {}", results[0]);
    assert!(results[0].contains("alpha"), "dbQuery should return the widget row: {}", results[0]);

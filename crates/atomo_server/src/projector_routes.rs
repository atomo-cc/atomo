...
async fn list_projections(State(manager): State<Arc<ProjectorManager>>) -> Json<Value> {
    let items: Vec<Value> = manager.projections().iter()
...
async fn rebuild_projections(State(manager): State<Arc<ProjectorManager>>) -> Result<Json<Value>, StatusCode> {
    match manager.rebuild_all().await {
        Ok(()) => Ok(Json(json!({ "status": "rebuilt", "count": manager.projections().len() }))),

    ...
    Session: Send + Sync,
{
    ...
    async fn authenticate(&self, credentials: &AuthCredentials) -> Result<Option<User>, Self::Error>;
    
    /// Authorize user for a specific resource and action
    async fn authorize(&self, user: &User, resource: &str, action: &str) -> Result<bool, Self::Error>;
    ...
    User: Send + Sync,
{
    ...
    /// Create a new user
    async fn create_user(&self, email: &str, password: &str, first_name: &str, last_name: &str, role: impl Send) -> Result<User, Self::Error>;

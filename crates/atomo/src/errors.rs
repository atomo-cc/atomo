    ...
    fn from(err: AtomoError) -> Self {
        match err {
            AtomoError::NotFound { model, id } => {
                GqlError::new(format!("{} with id '{}' not found", model, id))
                    .extend_with(|_, e| { e.set("code", "NOT_FOUND"); e.set("model", model.as_str()); })
            }
            AtomoError::Unauthorized { message } => {
                GqlError::new(message)
                    .extend_with(|_, e| { e.set("code", "UNAUTHORIZED"); })
            }
            AtomoError::Forbidden { message } => {
                GqlError::new(message)
            ...
            AtomoError::ValidationFailed { errors } => {
                ...
                GqlError::new(msg)
                    .extend_with(|_, e| {
                        ...
                        e.set("fields", serde_json::to_string(&field_errors).unwrap_or_default());
                    })
            }
            AtomoError::Internal { message } => {
                GqlError::new(message)

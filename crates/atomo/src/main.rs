...
async fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Generate { schema } => {
            ...
            println!(
                "🔧 Generating Hasura v2 GraphQL code from schema: {}",
                schema
            );
            ...
            let type_generator =
                atomo_schema::hasura_v2_type_generator::HasuraV2TypeGenerator::new();
            let types_code = type_generator.generate_types(&models)?;

            // Generate Hasura v2 resolvers
            let resolver_generator =
                atomo_schema::hasura_v2_resolver_generator::HasuraV2ResolverGenerator::new();
            ...
            println!(
                "✅ Hasura v2 resolvers generated at: {}",
                resolvers_output_path
            );
            Ok(())
        }
        Commands::Serve { port, schema } => serve_command(port, schema).await,
        Commands::Migrate { schema } => migrate_command(schema).await,
    }
}
...
async fn serve_command(port: u16, schema_path: String) -> Result<()> {
    ...
    println!(
        "✅ GraphQL endpoint available at http://localhost:{}/graphql",
        port
    );

...
use atomo_schema::{
    hasura_v2_type_generator::HasuraV2TypeGenerator, HasuraV2ResolverGenerator,
    OperationDefinitions, TypeScriptParser,
};
use colored::*;
use console::style;
...
pub async fn dev_command(
    ...
    println!(
        "   📁 Created runtime workspace: {}",
        runtime_dir.display().to_string().dimmed()
    );
    ...
    if effective_verify {
        ...
        println!(
            ...
            operation_definitions.comparison_ops.len()
        );
        println!(
            "      - {} logical operators",
            operation_definitions.logical_ops.len()
        );
    }
...
async fn generate_business_code(runtime_dir: &Path, schema_path: &Path) -> Result<()> {
    ...
    // 检查是否需要重新生成
    let schema_modified = schema_path
        .metadata()
        .and_then(|m| m.modified())
        .unwrap_or(std::time::UNIX_EPOCH);

    let models_modified = models_path
        .metadata()
        .and_then(|m| m.modified())
        .unwrap_or(std::time::UNIX_EPOCH);

    let resolvers_modified = resolvers_path
        .metadata()
        .and_then(|m| m.modified())
        .unwrap_or(std::time::UNIX_EPOCH);
    ...
    println!(
        "      ├─ Complexity: {}",
        calculate_complexity_score(&models)
    );
    println!(
        "      └─ Hash: {}...",
        &calculate_schema_hash(&schema_content)[..8]
    );
...
/// 保存构建缓存信息
async fn save_build_cache(
    runtime_dir: &Path,
    schema_content: &str,
    models: &[atomo_schema::Model],
) -> Result<()> {
...
async fn compile_and_run_service(runtime_dir: &Path, service_name: &str, port: u16) -> Result<()> {
    ...
    if !compile_status.success() {
        anyhow::bail!(
            "❌ Failed to compile service runtime. Please check the compilation errors above."
        );
    }
    ...
    println!(
        "🎉 {} {} {}",
        "".repeat(5),
        format!("{} Started Successfully!", service_name)
            .bright_green()
            .bold(),
        "".repeat(5)
    );
...
async fn schema_first_generation(runtime_dir: &Path, schema_path: &Path) -> Result<()> {
    ...
    // 步骤2: 解析schema.ts
    let schema_content =
        std::fs::read_to_string(schema_path).context("Failed to read schema.ts")?;
    ...
    let schema_graphql_content = type_generator
        .generate_graphql_schema_with_operations(&models, &operation_definitions)
    ...
    // 步骤4: 验证Schema一致性
    verify_schema_consistency_with_operations(&schema_graphql_path, &operation_definitions)
        .await
    ...
    let resolver_generator = HasuraV2ResolverGenerator::new();
    let rust_models_content = resolver_generator
        .generate_models_with_operations(&models, &operation_definitions)
        .context("Failed to generate Rust models with unified operations")?;

    let models_path = runtime_dir.join("src").join("models.rs");
    std::fs::write(&models_path, &rust_models_content).context("Failed to write models.rs")?;

    println!("   🦀 Generated models.rs with identical operation definitions");

    // 步骤6: 生成Resolver实现
    let resolver_content = resolver_generator
        .generate_resolvers_with_operations(&models, &operation_definitions)
        .context("Failed to generate resolvers with unified operations")?;

    let resolvers_path = runtime_dir.join("src").join("resolvers.rs");
    std::fs::write(&resolvers_path, &resolver_content).context("Failed to write resolvers.rs")?;

    println!("   🔗 Generated resolvers.rs with matching operation definitions");

    // 步骤7: 运行时一致性最终验证
    verify_runtime_schema_consistency(&schema_graphql_path, &models_path, &operation_definitions)
        .await
...
async fn verify_schema_consistency_with_operations(
    ...
    let content =
        std::fs::read_to_string(schema_graphql_path).context("Failed to read schema.graphql")?;
...
async fn verify_runtime_schema_consistency(
    ...
    let models_content =
        std::fs::read_to_string(models_path).context("Failed to read models.rs")?;
...
async fn check_incremental_changes(schema_path: &Path, runtime_dir: &Path) -> Result<bool> {
    ...
    for file in &required_files {
        if !file.exists() {
            println!(
                "   🔄 Missing generated file: {} - regenerating",
                file.display()
            );
            return Ok(true);
        }
...
async fn compile_and_run_service_with_watch(
    ...
    println!(
        "   👀 Watching: {}",
        schema_path.display().to_string().dimmed()
    );
    println!();
        ...
        if let Ok(event) = rx.recv_timeout(Duration::from_millis(100)) {
            match event {
                Ok(notify::Event {
                    ...
                    if time_since_last < Duration::from_millis(10000) {
                        // 增加到10秒以捕获延迟事件
                    ...
                    println!(
                        "🔄 {}",
                        "Schema change detected! Reloading...".bright_yellow()
                    );

...
- External workers & blob storage: ⚪ design only — durable event-sourced jobs pulled by trusted out-of-process workers (side-effect/orchestration/media workloads) + a first-class blob/asset store. No core code yet. See `/guide/advanced/workers-and-blobs-design`

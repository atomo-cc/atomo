...
- Durable jobs (external workers): 🟡 `JobStore` lease engine — event-sourced jobs with idempotent enqueue, `SELECT … FOR UPDATE SKIP LOCKED` lease/heartbeat/complete/fail, visibility-timeout reclaim, and retry/backoff/dead-letter; Postgres-tested (`jobs_store`). HTTP lease API + worker-token auth + worker SDK still TODO. See `/guide/advanced/workers-and-blobs-design`

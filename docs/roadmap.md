...
- Admin UI: ✅ dynamic rendering with aligned API client; list grid honors schema `ui.listView` (v0.5.11, incl. timestamp columns); list search/filters/bulk-delete wired to the real where API, enum `select()` fields render as dropdowns, mutation buttons role-gated from schema access rules, and a **real observability panel** (`/observability`: job-queue health + audit activity from live admin-gated endpoints). Note: `ui.listView` is metadata-schema only — the builder DSL doesn't parse a `ui` block yet
...
- Caching: bounded process-local list/point/count cache with TTL/TTI, model overrides, coalesced demand refresh, generation barriers and explicit multi-instance bypass. Guide: [/guide/caching](/guide/caching).
...
- Actions & Workers
  ...
  - ✅ Worker SDK (`@atomo-cc/worker-sdk`): lease/heartbeat/complete/fail loop, typed handlers
  - Worker → Rust CRUD API (Phase 2): let workers read/write data through the engine

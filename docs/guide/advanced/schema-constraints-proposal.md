...
- ✅ **Partial / conditional indexes** — **shipped**: `// @@unique([col]) WHERE <predicate>`
  and `// @@index([col]) WHERE <predicate>` emit `CREATE [UNIQUE] INDEX ... WHERE <predicate>`.
  The predicate is raw SQL over column names (snake_case, like `@@check`), e.g.

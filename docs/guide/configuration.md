...
- Rate limiting (per-IP token bucket): `RATE_LIMIT_RPS` (default 100), `RATE_LIMIT_WINDOW_SECS` (default 60). Client IP comes from `X-Forwarded-For` when present — set `ATOMO_TRUST_X_FORWARDED_FOR=false` on a directly-exposed server to rate-limit on the peer address instead.

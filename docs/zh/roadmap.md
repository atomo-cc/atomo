...
- 存储生命周期（v0.7.0）：有界缓存、可选模型历史与独立审计保留、持久化重放能力标记和管理员诊断接口。默认继续保存完整历史与审计；跨功能验证见仓库 `docs/implementation/storage-lifecycle/TODO.md`，发布验收见同目录 `RELEASE_TODO.md`。配置与限制见[存储生命周期](/guide/storage-lifecycle)。

...
Atomo Content Core は、コンテンツ駆動型アプリ向けの、オープンソースでセルフホスト可能な**イベントソース型バックエンド**です。TypeScript の `schema.ts` でデータモデルを定義すると、**GraphQL API**、**認証 + RBAC**、**リアルタイム**、自動生成の**管理 UI** が手に入ります。**アクションと外部ワーカー**で拡張でき、**Docker** でデプロイ可能（Rust ツールチェーン不要）。自分の Postgres 上で動く、セルフホスト型の **Firebase/Supabase 代替**と考えてください。
...
- 🔌 **Actions & Workers**: スキーマでライフサイクルアクションを宣言（`on.created`、`on.updated`）し条件を指定 — Atomo が自動的に外部 TypeScript ワーカーへ永続ジョブをディスパッチ
- 🧩 **フォーク不要の拡張**: 宣言的なスキーマ制約（`@unique` / `@@check` / 部分インデックス）＋ ダイレクトアクション API（`POST /api/actions/:name`）
...
- [x] Actions & workers: ライフサイクルイベントバインディング（`ModelEvents`）、アクションディスパッチャー、ダイレクトアクション API、Worker SDK（`@atomo-cc/worker-sdk`）
- [x] フォーク不要の拡張: 宣言的なスキーマ制約（`@unique`/`@index`/`@@check`、`WHERE` 付き部分インデックスを含む）
